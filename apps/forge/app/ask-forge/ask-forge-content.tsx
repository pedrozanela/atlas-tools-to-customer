...
export default function AskForgeContent() {
  ...
  const fetchSuggestions = React.useCallback((p: AssistantPersona) => {
    forgeFetch(`/api/assistant/suggestions?persona=${p}`)
  ...
  React.useEffect(() => {
    forgeFetch("/api/assistant/conversations")
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => {
  ...
  const fetchTableDetails = React.useCallback(async (fqns: string[]) => {
    ...
    await Promise.all(
      fqns.slice(0, 10).map(async (fqn) => {
        try {
          const resp = await forgeFetch(`/api/environment/table/${encodeURIComponent(fqn)}`, {
  ...
  const handleSelectConversation = React.useCallback(
    async (conversationId: string) => {
      ...
      try {
        const resp = await forgeFetch(`/api/assistant/conversations/${conversationId}`, {
          signal: controller.signal,
        });
  ...
  const handleClearOrDelete = React.useCallback(async () => {
    if (activeConversationId) {
      try {
        await forgeFetch(`/api/assistant/conversations/${activeConversationId}`, { method: "DELETE" });
      } catch {
        // best-effort
      }
