...
export default function AssessmentPage() {
  ...
  const refreshAssets = useCallback(async () => {
    try {
      const res = await forgeFetch("/api/assessment/assets", { cache: "no-store" });
      if (!res.ok) return;
  ...
  const refresh = useCallback(async () => {
    try {
      const res = await forgeFetch("/api/assessment", { cache: "no-store" });
  ...
  const runAssessment = useCallback(async () => {
    setRunning(true);
    try {
      const res = await forgeFetch("/api/assessment/run", {
        method: "POST",
  ...
  const generateDashboard = useCallback(async () => {
    ...
    try {
      const res = await forgeFetch("/api/assessment/dashboard", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const generateGenie = useCallback(async () => {
    ...
    try {
      const res = await forgeFetch("/api/assessment/genie", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const saveQualitative = useCallback(
    async (input: { wafId: string; response: WafQualitativeAnswer; notes: string | null }) => {
      const res = await forgeFetch("/api/assessment/qualitative", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const ignoreControl = useCallback(
    async (wafId: string) => {
      ...
      const res = await forgeFetch("/api/assessment/ignored", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
