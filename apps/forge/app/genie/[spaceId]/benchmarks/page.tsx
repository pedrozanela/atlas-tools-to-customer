...
export default function BenchmarkPage() {
  ...
  const fetchQuestions = useCallback(async () => {
    try {
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/benchmarks`);
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/benchmarks/history`);
      if (res.ok) {
        const data = await res.json();
  ...
  const runBenchmarks = async (questionIds?: string[]) => {
    ...
    try {
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/benchmarks/run`, {
        method: "POST",
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
          const pollRes = await forgeFetch(`/api/genie-spaces/${spaceId}/benchmarks/run?evalRunId=${data.evalRunId}`,
          );
  ...
  const submitFeedback = async () => {
    ...
    try {
      ...
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/benchmarks/feedback`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const runImprove = async () => {
    ...
    setImproving(true);
    try {
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/benchmarks/improve`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const runOptimize = async () => {
    ...
    setOptimizing(true);
    try {
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/benchmarks/optimize`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ benchmarkRunId: currentRunId }),
      });
      ...
      const mergeRes = await forgeFetch(`/api/genie-spaces/${spaceId}/benchmarks/merge`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleApply = async (serializedSpace: string) => {
    setApplying(true);
    try {
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/apply`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleCloneAndApply = async (serializedSpace: string) => {
    setCloning(true);
    try {
      const cloneRes = await forgeFetch(`/api/genie-spaces/${spaceId}/clone`, {
        method: "POST",
      });
      if (!cloneRes.ok) throw new Error("Clone failed");
      const { clonedSpaceId } = await cloneRes.json();

      const applyRes = await forgeFetch(`/api/genie-spaces/${clonedSpaceId}/apply`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ serializedSpace }),
      });
