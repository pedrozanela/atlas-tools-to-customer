...
export default function SpaceDetailPage() {
  ...
  const fetchDetail = useCallback(async () => {
    try {
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/detail`);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await forgeFetch(`/api/genie-spaces/auto-improve?jobId=${autoImproveJobId}`);
        if (!res.ok) return;
  ...
  const handleFix = async (checkIds: string[]) => {
    ...
    setFixResult(null);
    try {
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/fix`, {
        method: "POST",
  ...
  const pollDeployJob = async (jobId: string): Promise<string> => {
    for (let i = 0; i < 120; i++) {
      ...
      const pollRes = await forgeFetch(`/api/genie-spaces?deployJobId=${jobId}`);
  ...
  const handleClone = async () => {
    setCloning(true);
    try {
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/clone`, {
        method: "POST",
      });
  ...
  const pollImproveStatus = useCallback(async () => {
    try {
      const res = await forgeFetch(`/api/genie-spaces/${spaceId}/improve`);
      if (!res.ok) return;
  ...
  useEffect(() => {
    if (!spaceId) return;
    forgeFetch(`/api/genie-spaces/${spaceId}/improve`)
      .then((r) => r.json())
  ...
  const handleCancelImprove = async () => {
    try {
      await forgeFetch(`/api/genie-spaces/${spaceId}/improve`, { method: "DELETE" });
      setImproving(false);
      setImproveProgress(null);
      stopImprovePolling();
  ...
  const dismissImproveResult = useCallback(() => {
    setImproveResult(null);
    forgeFetch(`/api/genie-spaces/${spaceId}/improve?action=dismiss`, { method: "DELETE" }).catch(
      () => {},
    );
  }, [spaceId]);
