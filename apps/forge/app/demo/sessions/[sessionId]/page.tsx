...
export default function DemoSessionPage() {
  ...
  useEffect(() => {
    ...
    const fetchSession = async () => {
      try {
        const r = await forgeFetch(`/api/demo/sessions/${sessionId}`);
        if (cancelled) return;
  ...
  const handleLaunchDiscovery = useCallback(async () => {
    ...
    try {
      ...
      const execRes = await forgeFetch(`/api/runs/${runId}/execute`, {
        method: "POST",
      });
  ...
  const handleLaunchEstateScan = useCallback(async () => {
    ...
    try {
      const scope = `${session.catalogName}.${session.schemaName}`;
      const res = await forgeFetch("/api/environment-scan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ucMetadata: scope,
