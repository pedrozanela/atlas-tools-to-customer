...
export default function KnowledgeBasePage() {
  ...
  const fetchDocuments = React.useCallback(async () => {
    try {
      ...
      const resp = await forgeFetch("/api/knowledge-base");
      if (resp.ok) {
  ...
  const handleUpload = async (files: FileList | File[]) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const resp = await forgeFetch("/api/knowledge-base/upload", {
          method: "POST",
          body: formData,
        });
  ...
  const handleDelete = async (id: string, filename: string) => {
    try {
      const resp = await forgeFetch(`/api/knowledge-base?id=${id}`, { method: "DELETE" });
      if (resp.ok) {
