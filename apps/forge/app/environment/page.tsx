...
export default function EstatePage() {
  ...
  const fetchAggregate = useCallback(async () => {
    try {
      setLoading(true);
      const resp = await forgeFetch("/api/environment/aggregate");
  ...
  const fetchGovernanceQuality = useCallback(async () => {
    try {
      const resp = await forgeFetch("/api/stats");
      if (!resp.ok) return;
      const data = await resp.json();
      if (data?.quality) {
  ...
  const fetchAggregateErd = useCallback(async () => {
    try {
      const resp = await forgeFetch("/api/environment/aggregate/erd?format=json");
      if (resp.ok) {
  ...
  const loadSingleScan = useCallback(async (scanId: string) => {
    try {
      const resp = await forgeFetch(`/api/environment-scan/${scanId}`);
      if (!resp.ok) throw new Error("Not found");
      const scan = await resp.json();
      setSelectedScan(scan);
      setViewMode("single-scan");

      const erdResp = await forgeFetch(`/api/environment-scan/${scanId}/erd?format=json`);
      if (erdResp.ok) {
  ...
  const deleteScan = useCallback(
    async (scanId: string) => {
      try {
        const resp = await forgeFetch(`/api/environment-scan/${scanId}`, { method: "DELETE" });
        if (!resp.ok) {
    ...
    (scanId: string) => {
      ...
      const poll = async () => {
        ...
        try {
          ...
          const progResp = await forgeFetch(`/api/environment-scan/${scanId}/progress`);
          if (progResp.ok) {
            consecutiveMisses = 0;
            ...
            const scanResp = await forgeFetch(`/api/environment-scan/${scanId}`);
            if (scanResp.ok) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const resp = await forgeFetch("/api/environment-scan/active");
  ...
  const downloadExcel = useCallback(async (url: string, filename: string) => {
    setExporting(true);
    try {
      const resp = await forgeFetch(url);
