...
export default function AICommentsPage() {
  ...
  useEffect(() => {
    Promise.all([
      forgeFetch("/api/environment/comments").then((r) => (r.ok ? r.json() : { jobs: [] })),
      forgeFetch("/api/industries").then((r) => (r.ok ? r.json() : { industries: [] })),
  ...
  const loadJobData = useCallback(async (jobId: string) => {
    try {
      const res = await forgeFetch(`/api/environment/comments/${jobId}`);
      ...
      if (fqns.length > 0) {
        forgeFetch("/api/environment/comments/check-permissions", {
          method: "POST",
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const res = await forgeFetch("/api/environment/comments/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const startProgressPolling = useCallback(
    (jobId: string) => {
      ...
      const timer = setInterval(async () => {
        try {
          const res = await forgeFetch(`/api/environment/comments/${jobId}/progress`);
          if (!res.ok) {
            consecutiveMisses++;
  ...
  const reloadJobs = useCallback(async () => {
    try {
      const res = await forgeFetch("/api/environment/comments");
      if (res.ok) {
        const data = await res.json();
        setJobs(data.jobs ?? []);
      }
  ...
  const handleUpdateProposals = useCallback(
    async (updates: Array<{ id: string; status: string; editedComment?: string | null }>) => {
      ...
      try {
        const res = await forgeFetch(`/api/environment/comments/${activeJobId}/proposals`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const handleApplyAll = useCallback(async () => {
    ...
    setApplying(true);
    try {
      const res = await forgeFetch(`/api/environment/comments/${activeJobId}/apply`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleUndoAll = useCallback(async () => {
    if (!activeJobId) return;
    try {
      const res = await forgeFetch(`/api/environment/comments/${activeJobId}/undo`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ all: true }),
      });
  ...
  const handleResyncTable = useCallback(
    async (tableFqn: string) => {
      if (!activeJobId) return;
      try {
        const res = await forgeFetch(`/api/environment/comments/${activeJobId}/resync`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteJob = useCallback(async (jobId: string) => {
    try {
      await forgeFetch(`/api/environment/comments/${jobId}`, { method: "DELETE" });
