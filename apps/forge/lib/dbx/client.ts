 ...
 *   3. **App authorization (service principal)**: OAuth M2M via
 *      DATABRICKS_CLIENT_ID / DATABRICKS_CLIENT_SECRET injected at runtime.
 *   4. **CLI OAuth U2M (local dev)**: Shells out to `databricks auth token`
 ...
 *
 * The CLI is tried last on purpose. The Databricks Apps runtime injects
 * DATABRICKS_HOST, so an earlier CLI attempt looks viable in production and
 * shells out to a binary the container does not ship — logging a misleading
 * "run `databricks auth login`" warning before falling through to the SP.
 *
...
/** Exchange service-principal credentials for an OAuth M2M token. */
...
/** Error thrown when no auth mode could produce a token. */
function noCredentialsError(what: string): Error {
  const profile = process.env.DATABRICKS_CLI_PROFILE;
 ...
 *   4. CLI OAuth U2M – `databricks auth token` from the CLI's OAuth
...
export async function getBearerToken(): Promise<string> {
  ...
  throw noCredentialsError("No authentication credentials found.");
}
...
async function getAppBearerToken(): Promise<string> {
  ...
  // 2. OAuth M2M (Databricks Apps — service principal). Checked before the
  //    CLI so production cold starts don't exec a missing binary.
  const clientId = process.env.DATABRICKS_CLIENT_ID;
  const clientSecret = process.env.DATABRICKS_CLIENT_SECRET;
  if (clientId && clientSecret) return mintM2MToken(clientId, clientSecret);
  ...
  throw noCredentialsError("No app-level credentials found.");
}
