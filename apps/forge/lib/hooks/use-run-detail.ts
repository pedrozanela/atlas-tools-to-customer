...
export function useRunDetail(runId: string) {
  ...
  const fetchPromptLogs = useCallback(async () => {
    ...
    setLogsLoading(true);
    try {
      const res = await forgeFetch(`/api/runs/${runId}/prompt-logs`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleRerun = useCallback(async () => {
    ...
    try {
      ...
      const createRes = await forgeFetch("/api/runs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const execRes = await forgeFetch(`/api/runs/${newRunId}/execute`, { method: "POST" });
      if (!execRes.ok) {
  ...
  const enrichWithPbi = useCallback(async () => {
    ...
    try {
      const res = await forgeFetch(`/api/runs/${runId}/enrich-pbi`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        if (data.status === "generating") {
          ...
          if (!geniePollRef.current) {
            ...
            const geniePoll = async () => {
              try {
                const r = await forgeFetch(`/api/runs/${runId}/genie-engine/generate/status`);
                if (r.ok) {
        ...
        if (data.status === "generating") {
          ...
          if (!dashboardPollRef.current) {
            ...
            const dashPoll = async () => {
              try {
                const r = await forgeFetch(`/api/runs/${runId}/dashboard-engine/generate/status`);
                if (r.ok) {
                  const d = await r.json();
                  if (d.status !== "generating") {
