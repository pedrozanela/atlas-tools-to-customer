...
- `lib/pipeline/scheduler.ts` -- in-process scheduler that polls Lakebase
  ...
  winner), and starts them via the registered starter. Promotion requires a
  short-lived, process-local OBO handoff refreshed by the authenticated
  `/api/system-load` poll; tokens are never persisted, and a run remains
  queued after restart or expiry until its owner supplies fresh OBO context.
