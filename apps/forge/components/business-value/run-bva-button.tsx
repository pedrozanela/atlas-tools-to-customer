...
export function RunBvaButton() {
  ...
  const fetchRuns = useCallback(async () => {
    setLoading(true);
    try {
      const res = await forgeFetch("/api/runs?limit=50");
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await forgeFetch(`/api/runs/${selectedRunId}/business-value/rerun`, {
        method: "POST",
      });
      if (res.status === 409) {
      ...
      pollRef.current = setInterval(async () => {
        try {
          const d = await forgeFetch(`/api/runs/${selectedRunId}/business-value`);
          if (d.ok) {
