...
const navSections: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      // WAF Assessment moved out of Forge into apps/waf (mounted on /waf via Atlas shell).
      // The /assessment routes here remain functional but are no longer linked from the menu.
    ],
  },
  {
...
function useBenchmarksEnabled(): boolean {
  ...
  useEffect(() => {
    ...
    forgeFetch("/api/benchmarks/status")
      .then((r) => r.json())
      .then((data) => setEnabled(data.enabled ?? false))
      .catch(() => setEnabled(false));
  }, []);
...
export function SidebarNav() {
  ...
  return (
    <motion.aside
      data-atlas-tour="forge-navigation"
