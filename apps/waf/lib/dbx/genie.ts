 ...
 *
 * Uses Databricks Apps user authorization. Production Atlas never falls back
 * to its service principal for Genie reads or mutations.
 * fetchWithTimeout with TIMEOUTS.WORKSPACE.
 *
 * API docs: https://docs.databricks.com/api/workspace/genie
 */

import { getAppHeaders, getConfig, getHeaders } from "./client";
...
async function resolveHeaders(
  ...
  if (authMode === "sp") {
    if (process.env.WAF_DATABRICKS_AUTH_MODE === "obo") {
      throw new Error("Service-principal Genie authentication is disabled for Atlas WAF");
    }
...
export async function updateGeniePermissions(
  spaceId: string,
  accessControlList: Array<{
    user_name: string;
    permission_level: "CAN_RUN" | "CAN_MANAGE";
  }>,
  oboToken?: string,
): Promise<void> {
  if (!/^[0-9a-f]{32}$/.test(spaceId)) {
    throw new Error("Unsafe Genie Agent ID for permission update");
  }
  if (accessControlList.length === 0) return;

  const config = getConfig();
  const headers = await resolveHeaders(undefined, oboToken);
  const response = await fetchWithTimeout(
    `${config.host}/api/2.0/permissions/genie/${spaceId}`,
    {
      method: "PATCH",
      headers,
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Genie permission update failed (${response.status}): ${text}`);
  }
}
...
export async function createGenieSpace(opts: {
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    try {
      await mkdirs(parentPath, opts.oboToken);
    } catch (e) {
