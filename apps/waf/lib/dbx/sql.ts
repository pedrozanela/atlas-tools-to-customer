...
export interface ExecuteSQLOptions {
  ...
  /**
   * Request-scoped Databricks Apps user token. Background work must receive
   * this explicitly because Next.js request headers are unavailable after the
   * originating request has returned.
   */
  oboToken?: string;
}

function headersForToken(oboToken?: string): Promise<Record<string, string>> {
  if (oboToken) {
...
export async function executeSQL(
  ...
  const headers = await headersForToken(options?.oboToken);
  ...
  while (
    ...
    result = await pollStatement(result.statement_id, options?.oboToken);
    attempts++;
  }
  ...
  if (result.result?.next_chunk_internal_link) {
    ...
    while (nextLink) {
      const chunkHeaders = await headersForToken(options?.oboToken);
...
async function pollStatement(statementId: string, oboToken?: string): Promise<StatementResponse> {
  const config = getConfig();
  const url = `${config.host}/api/2.0/sql/statements/${statementId}`;
  const headers = await headersForToken(oboToken);
