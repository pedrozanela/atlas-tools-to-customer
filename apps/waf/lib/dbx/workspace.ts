 ...
 * Uses the request-scoped OBO token so every mutation is attributed to the
 * administrator using the Atlas App.
 */
 ...
 */
export async function mkdirs(path: string, oboToken?: string): Promise<void> {
  const config = getConfig();
  const url = `${config.host}/api/2.0/workspace/mkdirs`;

  const headers = oboToken
    ? {
