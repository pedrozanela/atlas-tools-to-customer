 ...
 */
/**
 * Mint an OAuth M2M token from the app's SP credentials.
 *
 * Cached in-process until ~60 s before expiry. Shared by getBearerToken
 * and getAppBearerToken so we only round-trip /oidc/v1/token once per
 * cache window.
 */
...
export async function getBearerToken(): Promise<string> {
  ...
  if (process.env.WAF_DATABRICKS_AUTH_MODE === "obo") {
    throw new Error(
      "Databricks OBO token is unavailable. Open the Atlas App as its authorized " +
        "admin and approve the requested user authorization scopes.",
    );
  }
  ...
  // 3. OAuth M2M (Databricks Apps — service principal). Checked before
  //    the CLI so production cold starts don't exec a missing binary.
  const clientId = process.env.DATABRICKS_CLIENT_ID;
  const clientSecret = process.env.DATABRICKS_CLIENT_SECRET;
  if (clientId && clientSecret) return mintM2MToken(clientId, clientSecret);

  // 4. Databricks CLI OAuth U2M (local dev — no credentials on disk)
...
/**
 * Obtain a Bearer token using only app-level credentials (PAT, SP, or CLI).
 * Deliberately skips the user's forwarded token.
 *
 * Priority order:
 *   1. PAT (explicit override for local dev / break-glass).
 *   2. OAuth M2M with SP creds — always available in Databricks Apps and
 *      is the production path. We check this before the CLI so prod logs
 *      don't get polluted by a CLI-not-found warning every cold start.
 *   3. CLI OAuth U2M — local dev fallback when no PAT/SP is set.
 */
async function getAppBearerToken(): Promise<string> {
  ...
  // 2. OAuth M2M (Databricks Apps — service principal). Preferred over CLI
  //    so production cold starts don't try to exec the missing `databricks`
  //    binary and log a misleading warning.
  const clientId = process.env.DATABRICKS_CLIENT_ID;
  const clientSecret = process.env.DATABRICKS_CLIENT_SECRET;
  if (clientId && clientSecret) {
    return mintM2MToken(clientId, clientSecret);
  }
