 ...
 * proxy in prod, otherwise `getBearerToken()` for local development only.
 * The captured token is the one that should be
...
export async function getUserOrNull(req?: Request): Promise<ForgeUser | null> {
  ...
  try {
    ...
    if (!email) {
      const proxyEmail = hdrs.get(X_FORWARDED_EMAIL) ?? hdrs.get(X_FORWARDED_PREFERRED_USERNAME);
  ...
  // Local development may use PAT/CLI credentials. Production is strict OBO:
  // silently substituting the App service principal would bypass the
  // authenticated administrator's Unity Catalog permissions.
  if (!oboToken && isDev()) {
    try {
