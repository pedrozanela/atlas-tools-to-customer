...
export async function runPillar(
  pillar: WafPillarWithQuery,
  oboToken: string,
): Promise<WafControlResult[]> {
  const sql = await loadPillarSql(pillar);
  const result = await executeSQL(sql, undefined, undefined, { oboToken });
...
export async function runAllPillars(oboToken: string): Promise<{
  results: WafControlResult[];
  errors: Array<{ pillar: WafPillar; message: string }>;
}> {
  const settled = await Promise.allSettled(
    WAF_PILLARS_WITH_QUERIES.map((p) => runPillar(p, oboToken)),
  );
