...
import { QUALITATIVE_SCORE, QUALITATIVE_THRESHOLD, WAF_PILLARS } from "./types";
...
function pillarScore(
  metByPillar: Map<WafPillar, { met: number; total: number }>,
  pillar: WafPillar,
): number | null {
...
export async function runAssessment(opts: {
  ...
  try {
    const { results, errors } = await runAllPillars(opts.oboToken);
    ...
    const responses = await withPrisma((prisma) => prisma.wafQualitativeResponse.findMany());
    ...
    const ignored = await withPrisma((prisma) =>
      prisma.wafIgnoredResource.findMany({
    ...
    await withPrisma((prisma) =>
      prisma.wafControlResult.createMany({
        data: valid.map((r) => ({
          assessmentId,
          wafId: r.wafId,
          pillar: r.pillar,
...
export async function getLatestAssessment(
  ...
  const latest = await withPrisma((prisma) =>
    prisma.wafAssessment.findFirst({
    ...
    results: assessment.results.map(
      (r: {
        ...
        control: {
          ...
          evaluationType: r.control.evaluationType === "qualitative" ? "qualitative" : "automatic",
        },
      }),
    ),
  };
...
export async function listQualitativeResponses(): Promise<WafQualitativeResponse[]> {
  const rows = await withPrisma((prisma) =>
    prisma.wafQualitativeResponse.findMany({ orderBy: { wafId: "asc" } }),
  );
...
export async function saveQualitativeResponse(input: {
  ...
  const row = await withPrisma((prisma) =>
    prisma.wafQualitativeResponse.upsert({
      where: { wafId: input.wafId },
      create: {
        wafId: input.wafId,
...
export async function deleteQualitativeResponse(wafId: string): Promise<void> {
  await withPrisma((prisma) => prisma.wafQualitativeResponse.deleteMany({ where: { wafId } }));
}
...
export async function addIgnoredResource(input: {
  ...
  const exists = await withPrisma((prisma) =>
    prisma.wafControl.findUnique({ where: { wafId: input.wafId } }),
  );
  ...
  const row = await withPrisma(async (prisma) => {
    const existing = await prisma.wafIgnoredResource.findFirst({
      where: { wafId: input.wafId, resourceType, resourceId },
    });
    if (existing) {
      return prisma.wafIgnoredResource.update({
    ...
    return prisma.wafIgnoredResource.create({
      data: {
        wafId: input.wafId,
        resourceType,
        resourceId,
...
export async function deleteIgnoredResource(id: string): Promise<void> {
  await withPrisma((prisma) => prisma.wafIgnoredResource.deleteMany({ where: { id } }));
}
