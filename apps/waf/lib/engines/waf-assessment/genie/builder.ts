...
export const LEGACY_WAF_GENIE_DESCRIPTION =
...
export function getSpOwnedWafGenieDescription(): string {
  const appName = process.env.ATLAS_APP_NAME?.trim() || "atlas";
  return (
    `Atlas-managed WAF Genie Agent owned by Databricks App "${appName}". ` +
    "Analyzes failing controls with curated WAF guidance and system-table evidence."
  );
}

export function getWafGenieDescription(): string {
  const appName = process.env.ATLAS_APP_NAME?.trim() || "atlas";
  return (
    `Atlas-managed WAF Genie Agent for Databricks App "${appName}", operated with user authorization. ` +
    "Analyzes failing controls with curated WAF guidance and system-table evidence."
  );
}
...
const TABLES: Array<Omit<DataSourceTable, "description"> & { description: string }> = [
  ...
  },
  {
    ...
    description:
      "Unity Catalog table-level lineage edges. Used to validate lineage coverage for DG-01-03.",
  },
  {
  ...
  },
  {
    ...
    description:
      "Historical Databricks SKU list prices. Join to billing usage on cloud, SKU and usage interval for cost evidence.",
  },
  {
  ...
  {
    identifier: "system.information_schema.table_tags",
    description:
      "Governed tags attached to Unity Catalog tables. Used to validate metadata and classification coverage.",
  },
  {
    identifier: "system.information_schema.row_filters",
    description:
      "Unity Catalog row filters. Used as evidence for centralized, fine-grained access controls.",
  },
  {
    identifier: "system.information_schema.routines",
    description:
      "Unity Catalog routines and functions. Used to inspect reusable SQL and AI-function adoption.",
  },
];
...
ORDER BY name`,
  ...
  {
    question:
      "Average CPU and memory utilisation per day for the last 30 days, by driver vs workers",
    sql: `SELECT
